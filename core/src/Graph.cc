...
void ComputationGraph::addDecl(const boost::uuids::uuid& path, const Configuration::ModuleDecl& decl) {
...
const std::vector<boost::uuids::uuid>& ComputationGraph::getPaths() const {
    return sorted_execution_paths;
}

const std::vector<Configuration::ModuleDecl>& ComputationGraph::getDecls(const boost::uuids::uuid& path) const {
...
std::shared_ptr<ComputationGraph> ComputationGraphBuilder::build() {
    ...
    if (n_dimensions < configuration.getNDimensions()) {
        ...
        configuration.setNDimensions(n_dimensions);
    }
